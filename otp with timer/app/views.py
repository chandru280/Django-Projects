    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user.email:
                ...
                OTP.objects.create(user=user, otp=otp_code, expiry_time=datetime.now() + timedelta(minutes=2))  # Set expiry time 2 minutes from now
...
def enter_otp(request):
    if request.method == 'POST':
        ...
        if otp_obj:
            ...
            if datetime.now() > otp_obj.expiry_time:
